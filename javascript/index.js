...
async function makeBrusselsSprouts() {
  try {
    ...
    for (let i = 0; i < steps; i++) {
      const currentStep = await obtainInstruction('brusselsSprouts', i);
      instructionsElement.innerHTML += `<li>${currentStep}</li>`;
    }

    const lastInstruction = instructionsElement.innerHTML += `<li> Brussels sprouts are ready! </li>`;
